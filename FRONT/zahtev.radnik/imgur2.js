...
(function (root, factory) {
    ...
    var Imgur = function (options) {
        ...
        this.dropzone = document.querySelector('.slika2').querySelectorAll('.dropzone');
        this.info = document.querySelector('.slika2').querySelectorAll('.info');
        //Start
        this.run();
    };
    ...
    Imgur.prototype = {
        ...
        loading: function () {
            ...
            img = this.createEls('img', {className: 'loading-image', src: '../media/loading-spin.svg'});
            
            div.appendChild(table);
            table.appendChild(img);
            document.body.querySelector('.slika2').appendChild(div);
        },
        status: function (el) {
        ...
        matchFiles: function (file, zone, fileCount) {
            ...
            if (file.type.match(/image/) && file.type !== 'image/svg+xml') {
                document.body.querySelector('.slika2').classList.add('loading');
                ...
                this.post(this.endpoint, fd, function (data) {
                    if (fileCount[0]+1 == fileCount[1]) {
                        document.body.querySelector('.slika2').classList.remove('loading');
                    }
        ...
        run: function () {
            var loadingModal = document.querySelector('.slika2').querySelector('.loading-modal');
